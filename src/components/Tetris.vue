...
onMounted(()=>{
    ...
    //3. board
    const board = new Array(board_height).fill(f).map(() => new Array(board_width).fill(f));

    // 4. piece player
    let n = Math.floor(board_width / 2 - Math.floor(Math.random() * 3))
    const pieces= [
        [
            [t,t],
            [t,t]
    ...
    const removeRows = () => {
        ...
        rowsToRemove.forEach(y => {
            board.splice(y,1);
            const newRow = Array(board_width).fill(f);
